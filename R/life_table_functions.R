...
le.summary <- function(death.df, population.df, n, age.cohort) {
  ...
  for (g in genders) {
    d.df <- death.df %>%
      filter(Gender == g)
    d.df <- d.df[match(age.cohort, d.df$Age.Group),]
    pop.df <- population.df %>%
      filter(Gender == g)
    pop.df <- pop.df[match(age.cohort, pop.df$Age.Group),]
    fit.est <- life.table(d.df$fit, pop.df$fit, n)[, 'Years.remaining']
    fit.est <- cbind(Gender = rep(g, length(fit.est)), Age.Group = age.cohort, Fit.Bound = rep('fit', length(fit.est)), LE = fit.est)
    lwr.est <- life.table(d.df$upr, pop.df$lwr, n)[, 'Years.remaining']
    lwr.est <- cbind(Gender = rep(g, length(lwr.est)), Age.Group = age.cohort, Fit.Bound = rep('lwr', length(lwr.est)), LE = lwr.est)
    upr.est <- life.table(d.df$lwr, pop.df$upr, n)[, 'Years.remaining']
    upr.est <- cbind(Gender = rep(g, length(upr.est)), Age.Group = age.cohort, Fit.Bound = rep('upr', length(upr.est)), LE = upr.est)
    lex.table <- rbind(lex.table, fit.est, lwr.est, upr.est)
  }
  return(lex.table)
}

ons.le.summary <- function(death.df, population.df, n, age.cohort) {
  # Calculates the life expectancy (fitted value, lower and upper bound) for all age cohorts for each gender
  # Inputs:
  #   death.df: data frame of the number of deaths reported by the ONS, need to have columns "Gender", "Deaths"
  #   population.df: data frame of estimated mid-year population, need to have columns "Gender", "fit", "lwr", and "upr" 
  #   n: vector of number of years each corresponding age.cohort spans
  #   age.cohort: vector (string) of names of age cohort in the correct order
  # Output:
